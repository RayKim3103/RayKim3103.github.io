...
**숫자 예 — LL 회전**: 빈 AVL tree에 `30, 20, 10`을 순서대로 삽입하면

```text
insert 30       insert 20        insert 10 (불균형!)
   30              30                  30
                   /                  /
                 20                 20
                                    /
...
`10`을 넣은 뒤 `30`에서 `BF(30) = height(right) - height(left) = -1 - 1 = -2`로 균형이 깨진다(왼쪽의 왼쪽이 무거운 **LL case**). Right rotation을 `30`에 적용하면
...
`20`이 새 root가 되고 균형이 회복된다(`BF(20)=0`). 만약 `10, 20, 30` 순서로 넣었다면 대칭인 **RR case**(오른쪽의 오른쪽이 무거움)가 되어 Left rotation이 필요하다.
...
- `30, 20, 10`을 순서대로 삽입했을 때 왜 LL case가 되고, right rotation 후 트리가 어떻게 바뀌는지 그릴 수 있는가?
- `10, 20, 30` 순서로 삽입하면 어떤 case(RR)가 되고 어떤 회전이 필요한가?
- 정렬된 순서로 키를 삽입하면 일반 BST가 왜 연결 리스트처럼 기울어지는지 설명할 수 있는가?
