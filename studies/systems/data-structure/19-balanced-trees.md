...
**숫자 예**: $$N=10^6$$개 레코드를 order $$M=100$$인 B-tree(노드당 최대 100개 자식)에 저장하면 높이는 대략

$$
\log_{100}(10^6) = \frac{\log_{10}10^6}{\log_{10}100} = \frac{6}{2} = 3
$$

즉 **디스크 접근 단 3번**으로 백만 개 중 원하는 레코드를 찾을 수 있다. 같은 데이터를 이진 트리(각 노드 자식 2개)에 넣으면 높이가 $$\log_2(10^6)\approx20$$이 되어 접근이 훨씬 잦다 — B-tree가 "노드당 자식 수를 늘려 트리 높이를 줄인다"는 설계 목표를 숫자로 보여준다.
...
- $$N=10^6$$, $$M=100$$인 B-tree의 높이를 계산하고, 같은 데이터를 이진 트리에 넣었을 때와 비교할 수 있는가?
- 2-3-4 tree에서 4-node에 key를 하나 더 넣으면 어떤 일이 일어나는지(split과 parent로 올라가는 key) 설명할 수 있는가?
- Red-black tree의 다섯 속성이 왜 높이를 $$O(\log n)$$으로 제한하는지 설명할 수 있는가?
