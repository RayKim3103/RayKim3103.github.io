...
directed graph: $$s\to1(4)$$, $$s\to2(1)$$, $$2\to1(2)$$, $$1\to3(1)$$, $$2\to3(5)$$에서 Dijkstra를 실행하면

| 단계 | `S`에 추가 | `dist[1]` | `dist[2]` | `dist[3]` |
|---|---|---:|---:|---:|
| 0 | — | $$\infty$$ | $$\infty$$ | $$\infty$$ |
| 1 | `s` (dist 0) | 4 (via `s`) | 1 (via `s`) | $$\infty$$ |
| 2 | `2` (dist 1) | **3** (via `2`, $$1{+}2{<}4$$) | — | 6 (via `2`) |
| 3 | `1` (dist 3) | — | — | **4** (via `1`, $$3{+}1{<}6$$) |
| 4 | `3` (dist 4) | — | — | — |

최종 `dist = {1:3, 2:1, 3:4}`. 흥미로운 점은 `s→1` 직접 간선의 weight가 4인데도 최단 경로는 `s→2→1`(총 $$1+2=3$$)이 더 짧다는 것 — **직접 연결이 항상 최단은 아니며**, `2`를 확정한 뒤 그 edge로 `1`을 relax해야 이 개선을 찾을 수 있다. 마찬가지로 `3`도 `2`를 거친 경로(6)보다 `1`을 거친 경로($$3+1=4$$)가 더 짧다.
...
- 위 4-vertex 예제에서 Dijkstra를 직접 추적해 `dist[1]=3`, `dist[3]=4`가 나오는 과정을 설명할 수 있는가?
- `s→1` 직접 간선(weight 4)보다 `s→2→1`(weight 3)이 더 짧은 이유와, 이것이 relaxation에서 왜 자동으로 반영되는지 설명할 수 있는가?
- MST와 shortest path tree가 왜 일반적으로 서로 다른 tree인지 설명할 수 있는가?
