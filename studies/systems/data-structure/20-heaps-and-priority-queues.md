...
**숫자 예**: `[4, 10, 3, 5, 1]`에 heapify를 적용하면(마지막 internal node index는 $$\lfloor5/2\rfloor-1=1$$)

| 단계 | 처리 노드 | 배열 |
|---|---|---|
| 시작 | — | `[4,10,3,5,1]` |
| `shiftDown(1)` | `10`(children `5,1`) — 이미 가장 큼, 변화 없음 | `[4,10,3,5,1]` |
| `shiftDown(0)` 1차 | `4`(children `10,3`) → `10`과 swap | `[10,4,3,5,1]` |
| `shiftDown(0)` 계속 | index1의 `4`(children `5,1`) → `5`와 swap | `[10,5,3,4,1]` |

최종 max-heap `[10,5,3,4,1]`(부모가 항상 자식 이상: $$10\ge5,3$$; $$5\ge4,1$$).
...
**힙 정렬 이어가기**: 위 `[10,5,3,4,1]`에서 root와 마지막 유효 원소를 반복해서 교환·축소하면

| 단계 | root ↔ 마지막 교환 후 | shiftDown 후 (heap 부분) | 정렬 완료 영역 |
|---|---|---|---|
| 1 | `[1,5,3,4,10]` | `[5,4,3,1]` | `10` |
| 2 | `[1,4,3,5,10]` | `[4,1,3]` | `5,10` |
| 3 | `[3,1,4,5,10]` | `[3,1]` | `4,5,10` |
| 4 | `[1,3,4,5,10]` | `[1]` | `3,4,5,10` |

최종 배열 `[1,3,4,5,10]`이 오름차순으로 정렬된다 — root(최댓값)를 매번 뒤쪽 미정렬 경계로 보내는 것이 heap sort의 핵심이다.
...
- `[4, 10, 3, 5, 1]`의 heapify 과정을 직접 추적해 `[10,5,3,4,1]`을 얻을 수 있는가?
- 그 heap으로 heap sort를 계속 진행해 `[1,3,4,5,10]`이 나오는 과정을 설명할 수 있는가?
- Binary heap이 왜 `insert`/`deleteMax`는 `O(log n)`이면서 `max()`는 `O(1)`인지 설명할 수 있는가?
