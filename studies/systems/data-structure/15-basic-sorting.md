...
**Trace**: `[5, 2, 4, 6, 1, 3]`에 삽입 정렬을 적용하면(`i`는 새로 삽입할 원소, 밑줄 부분이 정렬된 구간)
...
| 1 | 2 | `[2, 5, 4, 6, 1, 3]` |
| 2 | 4 | `[2, 4, 5, 6, 1, 3]` |
| 3 | 6 | `[2, 4, 5, 6, 1, 3]` (이미 제자리) |
| 4 | 1 | `[1, 2, 4, 5, 6, 3]` |
| 5 | 3 | `[1, 2, 3, 4, 5, 6]` |

`i=3`(`key=6`)에서는 왼쪽 정렬 구간의 마지막 값(`5`)보다 크므로 비교 한 번만 하고 아무것도 옮기지 않는다 — 이미 정렬된 위치에 있는 원소일수록 삽입 정렬의 비용이 싸지는 이유가 이 단계에서 드러난다. 반대로 `i=4`(`key=1`)는 왼쪽 정렬 구간 전체(4칸)를 오른쪽으로 밀어야 해 가장 비용이 크다.
...
- `[5, 2, 4, 6, 1, 3]`에 삽입 정렬을 직접 손으로 추적할 수 있는가?
- 왜 `key=6`일 때는 이동이 없고 `key=1`일 때는 4칸을 옮겨야 하는지 설명할 수 있는가?
- 버블 정렬, 선택 정렬, 삽입 정렬 중 어떤 것이 "거의 정렬된 입력"에 가장 강한지, 그 이유는?
