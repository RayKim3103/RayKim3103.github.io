...
스택의 대표 응용은 후위 표기식(postfix) 계산이다. `3 4 + 2 *`(중위로는 `(3+4)*2`)를 왼쪽부터 읽으며 숫자는 push, 연산자는 top 두 값을 pop해 계산한 뒤 다시 push한다.

| 입력 토큰 | 동작 | 스택(top이 오른쪽) |
|---|---|---|
| `3` | push | `[3]` |
| `4` | push | `[3, 4]` |
...
토큰을 모두 처리하면 스택에 결과 `14`만 남는다 — 괄호 없이도 연산 순서가 스택 구조 자체로 표현되는 것이 후위 표기식의 핵심이다.
...
- `3 4 + 2 *`의 후위 표기식 계산을 스택으로 직접 추적할 수 있는가?
- 벡터 기반 스택과 연결 리스트 기반 스택의 성능 trade-off를 설명할 수 있는가?
- 재귀 함수 호출이 왜 스택 구조(call stack)로 구현되는지 설명할 수 있는가?
