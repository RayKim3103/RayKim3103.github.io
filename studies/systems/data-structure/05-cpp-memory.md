...
**숫자 예**: `int a[4]`가 주소 `0x1000`부터 시작한다고 하자(`int`는 4바이트). `int* p = a;`이면
...
`p+1`이 `0x1001`이 아니라 `0x1004`인 이유는, 포인터 덧셈이 바이트 단위가 아니라 **가리키는 타입의 크기**(`sizeof(int)=4`) 단위로 이동하기 때문이다. `char*`였다면 `p+1`은 `0x1001`이 된다.
...
- `int* p`가 `0x1000`을 가리킬 때 `p+2`의 실제 주소를 계산할 수 있는가? `char*`라면 어떻게 달라지는가?
- `int* const p`와 `const int* p`의 의미 차이를 설명할 수 있는가?
- `new[]`로 할당한 메모리를 `delete`(배열 아님)로 해제하면 왜 문제가 되는가?
