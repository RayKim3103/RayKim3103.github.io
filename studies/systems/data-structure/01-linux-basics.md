...
**추적 예**: 위 Makefile에서 `main.o`가 없고 `main.cpp`만 있는 상태로 `make`를 실행하면, 규칙 `%.o: %.cpp`가 `$@ = main.o`, `$< = main.cpp`로 치환되어 `g++ -std=c++14 -Wall -c main.cpp`가 실행된다. `list.o`도 같은 규칙으로 만들어진 뒤, `main` 규칙에서 `$@ = main`, `$^ = main.o list.o`로 치환되어 `g++ -std=c++14 -Wall -o main main.o list.o`가 실행된다 — 파일 하나만 고쳐도 `make`가 그 파일에 의존하는 target만 다시 빌드하는 이유가 바로 이 의존성 그래프다.
...
- Makefile에서 `main.cpp`만 수정했을 때 `$@`, `$<`, `$^`가 각각 어떻게 치환되어 어떤 명령이 실행되는지 추적할 수 있는가?
- 하드 링크와 심볼릭 링크의 차이, 그리고 원본 파일을 지웠을 때 각각 어떻게 되는지 설명할 수 있는가?
- 전처리·컴파일·어셈블·링크 네 단계 중 헤더 파일 선언과 `.cpp` 정의 분리가 어느 단계와 관련 있는가?
