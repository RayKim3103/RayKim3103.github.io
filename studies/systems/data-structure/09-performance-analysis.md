...
**숫자 예 — 성장률의 실제 의미**: 한 연산에 1 마이크로초가 걸린다고 가정하고 $$N=10^6$$일 때 각 성장률의 실행 시간을 비교하면

| 성장률 | 연산 수 | 실행 시간(추정) |
|---|---:|---|
| $$O(\log N)$$ | $$\approx20$$ | $$20\,\mu s$$ |
| $$O(N)$$ | $$10^6$$ | $$1$$초 |
| $$O(N\log N)$$ | $$\approx2\times10^7$$ | $$20$$초 |
| $$O(N^2)$$ | $$10^{12}$$ | 약 11.6일 |

같은 하드웨어에서 $$O(N)$$과 $$O(N^2)$$의 차이는 $$N=100$$일 때는 1만 배에 불과하지만 $$N=10^6$$에서는 **$$10^6$$배**로 벌어진다 — 입력이 커질수록 성장률의 차이가 상수 계수 차이를 압도한다는 것이 성능 분석에서 성장률을 중시하는 이유다.
...
- $$N=10^6$$에서 $$O(N)$$과 $$O(N^2)$$ 알고리즘의 실행 시간 차이를 어림 계산할 수 있는가?
- 선택 정렬의 비교 횟수가 $$n(n-1)/2$$인 이유를 이중 반복문 구조로 설명할 수 있는가?
- Big-O, Big-Omega, Big-Theta의 차이(상한/하한/일치)를 설명할 수 있는가?
