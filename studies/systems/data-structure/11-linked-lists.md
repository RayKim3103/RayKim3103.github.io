...
**Trace (단일 연결 리스트)**: 빈 리스트에서 시작해 다음을 순서대로 실행하면
...
| `push_front(1)` | `1 → 5 → 10 → 20` |
| `pop_front()` | `5 → 10 → 20` |
| tail 삭제 | `5 → 10` |

마지막 tail 삭제에서는 `head`부터 `10`까지 순회해 "`tail` 바로 앞 노드"를 찾아야 한다(`10.next == tail`이 될 때 멈춤) — 원소가 많을수록 이 탐색이 길어져 `O(n)`이 되는 이유가 이 추적에서 바로 보인다. 이중 연결 리스트라면 `tail->prev`로 즉시 이 노드를 알 수 있어 `O(1)`이다.
...
- 위 trace를 손으로 재현해 각 단계의 리스트 상태를 그릴 수 있는가?
- 단일 연결 리스트의 tail 삭제가 `O(n)`인 이유와, 이중 연결 리스트가 `O(1)`로 만드는 원리를 설명할 수 있는가?
- 빈 리스트에서 마지막 원소를 삭제하면 왜 `head`와 `tail`을 모두 `nullptr`로 되돌려야 하는가?
