...
### 숫자로 확인하기 — 0.25초 delay 값이 실제로 맞는지 검증

측정된 기준값에서 반복당 시간을 역산하면

$$
\frac{1.25\text{s}}{100{,}000{,}000\text{회}} = 12.5\text{ns/회}
$$

Quiz에서 delay loop를 `20,000,000`회로 줄였으므로 예상 소요 시간은

$$
20{,}000{,}000 \times 12.5\text{ns} = 250{,}000{,}000\text{ns} = 0.25\text{s}
$$

측정 기준(1.25초/1억회)이 선형적으로 그대로 유지된다고 가정하면, 반복 횟수를 $$100{,}000{,}000 \to 20{,}000{,}000$$으로 정확히 $$1/5$$로 줄인 것이 지연 시간도 $$1.25\text{s} \to 0.25\text{s}$$로 정확히 $$1/5$$로 줄이는 결과와 맞아떨어진다 — Quiz가 목표로 한 "약 0.25초 간격"이 단순 비례식으로 정확히 재현되는 것을 확인할 수 있다.

### REG_A~REG_H 문자 용량 검증

$$
8\text{개 register} \times 4\text{문자/register} = 32\text{문자} = 2\text{행} \times 16\text{문자}
$$

이 계산이 Text-LCD의 실제 사양(2행×16문자)과 정확히 일치하므로, register map 설계가 LCD 물리 사양에 맞춰 정확히 맞춰졌음을 확인할 수 있다.

## 복습 질문

- 1.25초/1억회라는 측정 기준으로부터 2천만 회 반복 시 예상 지연이 0.25초임을 비례식으로 유도할 수 있는가?
- `8 register × 4문자 = 32문자 = 2행×16문자` 계산이 register map 설계와 LCD 사양의 일치를 어떻게 검증해주는지 설명할 수 있는가?
- 문자 packing 코드에서 `up_line[i*4]`가 왜 최상위 byte(`<<24`)에 배치되는지 big-endian 방식 packing 관점에서 설명할 수 있는가?
