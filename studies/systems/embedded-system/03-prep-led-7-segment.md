...
위 표의 8자리 구성이 왜 정확히 32비트인지 직접 세어보면: 자리 하나당 4비트(BCD 숫자 하나, 구분자 포함)이고 자리가 8개이므로 $$4\text{bit} \times 8 = 32\text{bit}$$다. `segdata[31:28]`부터 `segdata[3:0]`까지 8개의 4비트 slice로 정확히 나뉘어 남거나 모자란 비트가 없다 — 이는 이후 `seven_seg.v`가 `data[31:0]`를 4비트씩 8개로 기계적으로 분리할 수 있는 이유이기도 하다.

## 복습 질문

- `segdata`가 왜 32비트여야 하는지, 자리 수와 자리당 비트 수로 설명할 수 있는가?
- Common Anode와 Common Cathode 7-segment가 "켜지는 조건"에서 정확히 어떻게 반대인지 설명할 수 있는가?
- multiplexing 방식이 왜 8개 자리를 동시에 독립 구동하는 것보다 배선/자원 측면에서 유리한지 설명할 수 있는가?
