...
## 숫자로 확인하기 — 55x33이 60x34 대비 얼마나 여유를 두는가

이론상 최대 격자는 6주차 예비에서 구한 $$60 \times 34 = 2040$$문자다. 실제 사용된 안정 범위 55x33은

$$
55 \times 33 = 1815\text{문자}
$$

로, 이론값 대비 사용 비율은

$$
\frac{1815}{2040} \approx 89.0\%
$$

즉 가장자리 약 11%를 여백으로 남겨 화면 경계의 문자 잘림을 피한 것이다. 가로 방향으로는 $$60-55=5$$열, 세로 방향으로는 $$34-33=1$$행만큼 여유를 둔 셈이며, 가로 여유가 세로보다 큰 것은 "화면 가장자리 문자 잘림"이 세로보다 가로 active video 경계에서 더 민감하게 나타났음을 시사한다.
...
- $$55 \times 33 = 1815$$와 이론상 최대 $$2040$$을 비교해 실제 사용 비율(약 89%)을 계산할 수 있는가?
- 가로 여유(5열)가 세로 여유(1행)보다 큰 이유를 active video timing과 연결해 설명할 수 있는가?
- `chargen_rom_address = {ascii_code[7:0], subchar_line[2:0]}`에서 왜 ASCII 코드가 상위 비트, `subchar_line`이 하위 비트에 오는지 설명할 수 있는가?
