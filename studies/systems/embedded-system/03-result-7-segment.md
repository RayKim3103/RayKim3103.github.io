...
## 숫자로 확인하기 — 190Hz와 12,499,999는 어디서 나오는가

**digit multiplexing 주파수**: 한 digit이 `clk_cnt` 16384 cycle 동안 켜져 있고, 8개 digit을 모두 한 바퀴 돌려야 전체 화면이 한 번 갱신된다. 즉 한 프레임에 $$16384 \times 8 = 131072$$ cycle이 걸린다. 25MHz clock에서 이 cycle 수가 걸리는 시간은

$$
\frac{131072}{25{,}000{,}000\,\text{Hz}} \approx 5.24\text{ms}
$$

따라서 전체 화면 refresh 주파수는

$$
\frac{25{,}000{,}000}{16384\times 8} = \frac{25{,}000{,}000}{131072} \approx 190.7\text{Hz}
$$

로 노트에서 말한 "≈190Hz"가 정확히 재현된다.

**1Hz 생성**: `cnt == 12,499,999`에서 toggle하므로 `cnt`는 0부터 12,499,999까지 정확히 $$12{,}500{,}000$$번 세고 나서 `onesec_clk`을 뒤집는다. 25MHz clock에서 그 시간은

$$
\frac{12{,}500{,}000}{25{,}000{,}000\,\text{Hz}} = 0.5\text{s}
$$

즉 0.5초마다 한 번씩 toggle하므로, `onesec_clk`이 low로 0.5초 + high로 0.5초 = 총 1초 주기의 신호가 된다. $$25{,}000{,}000 \div 2 = 12{,}500{,}000$$이라는 절반 관계가 그대로 "1초를 만들려면 반주기가 0.5초여야 한다"는 요구사항과 맞아떨어진다.

**플리커와의 여유**: 사람 눈의 flicker-fusion 임계 주파수는 일반적으로 약 50~60Hz로 알려져 있다(그 이하로 깜빡이면 눈이 깜빡임을 인지한다). 이 회로의 190.7Hz는 그 임계치의 약 $$190.7 / 55 \approx 3.5$$배에 해당해 플리커 없이 보이는 데 충분한 여유가 있다 — "개선 및 고찰"에서 지적한 "더 낮은 refresh로도 충분하다"는 관찰이 이 숫자로 뒷받침된다.
...
- $$16384 \times 8 = 131072$$ cycle과 25MHz로부터 190.7Hz를 직접 유도할 수 있는가?
- `cnt == 12,499,999`가 왜 "0.5초"를 의미하고, 두 번 toggle되어야 1초 주기가 완성되는 이유를 설명할 수 있는가?
- 190.7Hz가 flicker-fusion 임계 주파수(약 50~60Hz)보다 훨씬 높다는 사실이 "refresh를 더 낮춰 전력을 아낄 여지가 있다"는 고찰과 어떻게 연결되는지 설명할 수 있는가?
- 14주차 결과의 sevenseg 모듈에서도 같은 `16384×8` multiplexing 구조가 재사용되는데, 그때 구동 clock이 여전히 25MHz인지 어떻게 확인할 수 있는가? ([14주차 결과](14-result-sevenseg-driver-8-byte-read-write.md) 참고)
