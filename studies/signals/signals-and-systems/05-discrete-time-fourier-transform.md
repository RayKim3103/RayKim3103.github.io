...
**숫자 예**: $$x[n]=(0.5)^nu[n]$$이면 $$X(e^{j\Omega})=1/(1-0.5e^{-j\Omega})$$.

$$
X(e^{j0}) = \frac{1}{1-0.5}=2,\qquad X(e^{j\pi})=\frac{1}{1-0.5(-1)}=\frac{1}{1.5}\approx0.667
$$

$$\Omega=0$$(DC)에서의 값 $$2$$는 $$\sum_n x[n]=\sum_{n=0}^\infty 0.5^n=1/(1-0.5)=2$$와 정확히 같다 — DTFT를 $$\Omega=0$$에서 계산하는 것은 항상 전체 sequence의 합을 구하는 것과 같기 때문이다. $$\Omega=\pi$$(가장 빠르게 진동하는 성분)에서는 $$0.667$$로 더 작다 — 이 신호가 저주파 위주(마지막 항으로 갈수록 부호가 계속 바뀌는 고주파 성분은 적음)이기 때문이다.
...
- $$x[n]=(0.5)^nu[n]$$의 $$X(e^{j0})$$이 왜 항상 $$\sum_n x[n]$$과 같은지 설명할 수 있는가?
- DTFT가 $$2\pi$$ 주기를 갖는 이유와, 이것이 CTFT와 다른 핵심 차이임을 설명할 수 있는가?
- DT LCCDE에서 주파수 응답 $$H(e^{j\Omega})$$를 계수들로부터 바로 쓰는 절차를 설명할 수 있는가?
